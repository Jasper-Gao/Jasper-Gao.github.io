---
title: "Conditionsl Probability and Independence"
date: "2026-09-15"
tag: "Math & CS"
...
（b）映射 $A\to P(A\mid B)$，从 $\mathcal{A}$ 映射到 $[0,1]$，定义了 $\mathcal{A}$ 上的一个新的概率测度，称为“**\*\*给定 $B$ 的条件概率测度（conditional probability measure given $B$）\*\***”。
...
$$Q\left(\bigcup\\\_{n=1}^{\infty}A\\\_n\right)=P\left(\left.\bigcup\\\_{n=1}^{\infty}A\\\_n\right|B\right)=\frac{P\left(\left(\bigcup\\\_{n=1}^{\infty}A\\\_n\right)\cap B\right)}{P(B)}=\frac{P\left(\bigcup\\\_{n=1}^{\infty}(A\\\_n\cap B)\right)}{P(B)}.$$

并且，由于序列 $(A\\\_n\cap B)\\\_{n\geq1}$ 两两不相交，

$$Q\left(\bigcup\\\_{n=1}^{\infty}A\\\_n\right)=\sum\\\_{n=1}^{\infty}\frac{P(A\\\_n\cap B)}{P(B)}=\sum\\\_{n=1}^{\infty}P(A\\\_n\mid B)=\sum\\\_{n=1}^{\infty}Q(A\\\_n).$$
...
$$P(A\\\_1\cap\cdots\cap A\\\_n)=P(A\\\_1)P(A\\\_2\mid A\\\_1)P(A\\\_3\mid A\\\_1\cap A\\\_2)\cdots P(A\\\_n\mid A\\\_1\cap\cdots\cap A\\\_{n-1}).$$
...
$$P(B)=P(A\\\_1)P(A\\\_2\mid A\\\_1)\cdots P(A\\\_{n-1}\mid A\\\_1\cap\cdots\cap A\\\_{n-2}),$$
...
$$A=A\cap\Omega=A\cap\left(\bigcup\\\_nE\\\_n\right)=\bigcup\\\_n(A\cap E\\\_n).$$

由于 $E\\\_n$ 两两不相交，因此 $(A\cap E\\\_n)\\\_{n\geq1}$ 也两两不相交，所以

$$P(A)=P\left(\bigcup\\\_n(A\cap E\\\_n)\right)=\sum\\\_nP(A\cap E\\\_n)=\sum\\\_nP(A\mid E\\\_n)P(E\\\_n).$$
